    ...
    if message.from_user.id == config.developer_id:
        try:
            ...
            await bot.reply_to(message,
                               ...
                               , parse_mode='HTML')
            await build_and_archive_solution(message_for_edit, 'Debug', config.solution_file)
...
async def build_and_archive_solution(message, configuration, solution_path, archive_name='sakuragram.zip'):
    ...
    await bot.send_document(config.channel_id, InputFile(open(archive_path, 'rb'), f"sakuragram_Windows_{configuration}_{current_time}.zip"))
